...
async function main(): Promise<void> {
  ...
  let chef = createChef({ dbPath, projectDir });
  await chef.start();
  ...
  let reopened = createChef({ dbPath, projectDir });
  ...
  process._getActiveHandles().forEach((x, i) => console.error(`  [${i}] ${x.constructor?.name} _handle=${x._handle?.constructor?.name ?? 'null'} _isStdio=${x._isStdio ?? 'N/A'}`));

  try {
    await rm(projectDir, { recursive: true, force: true });
    console.error("rm OK");
  } catch (e: unknown) {
    console.error(`rm FAILED: ${(e as Error).message}`);
  }
}
