...
function terminalMissionRecoveryMessage(status: "completed" | "failed" | "cancelled"): string {
  if (status === "cancelled") {
    return "This Mission was cancelled, so its history is final. Continue it as new work instead of retrying this step in place.";
  }
  if (status === "failed") {
    return "This Mission has already failed, so this attempt is final. Fix or continue it as new work instead of retrying this step in place.";
  }
