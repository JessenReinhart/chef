...
It creates a temporary local project, submits the permanent boring acceptance task through Chef's HTTP chat surface, waits for the Mission to finish, then starts the generated app with `npm start` and verifies that `/` serves a todo UI.
...
Failed runs always print and preserve the temporary project path so the generated files and `chef.sqlite` can be inspected directly.

## What this proves

A passing run proves the selected-project boundary, HTTP task submission, real planner, real task-capable worker routing, durable Task/Session completion, generated-file discoverability, documented `npm start` contract, and a reachable generated application in one executable scenario.
