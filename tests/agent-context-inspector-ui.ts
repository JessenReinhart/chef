...
const inspector = await readFile(new URL("../web/src/InspectorPanel.tsx", import.meta.url), "utf8");
const context = await readFile(new URL("../web/src/AgentContextInspector.tsx", import.meta.url), "utf8");

assert.match(inspector, /node\.kind === "agent" && <AgentContextInspector/, "agent nodes should expose the context inspector");
...
assert.match(context, /candidate\.memberNodeIds\.includes\(nodeId\)/, "only explicitly inherited Shared Context zones should be shown");
assert.match(context, /task\?\.contextRefs/, "task-specific context additions should remain inspectable");
