  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      if (threadHistory.isCurrent(selectionSnapshot)) {
        setStateRefreshError(stateRefreshErrorMessage(
          err instanceof Error ? err : new Error("Chef could not refresh the workspace"),
        ));
      }
    }
  }, [threadHistory]);
          ...
          {displayedError && (
            <div className="mt-3 rounded-xl border border-rose-400/20 bg-rose-400/[0.06] px-4 py-3 text-left text-xs text-rose-300" role="alert">
              ...
              <button
                ...
                className="mt-2 text-[10px] font-medium text-rose-200/70 transition hover:text-rose-100"
              >
                Dismiss
              </button>
            </div>
          ...
          <section className="mt-12 grid gap-4 lg:grid-cols-[1.25fr_0.75fr]">
            <div className="rounded-2xl border border-white/[0.08] bg-white/[0.025] p-5">
              ...
              {recentMissionActivity.length > 0 && (
                <div className="mt-4 rounded-xl border border-white/[0.06] bg-black/15 px-4 py-3 text-left" aria-label="Current Mission recent activity">
                  ...
                  <div className="mt-2 space-y-2">
                    {recentMissionActivity.map((activity, index) => (
                      <div key={`${index}-${activity}`} className="flex gap-2 text-xs leading-5 text-zinc-400">
                        ...
                        <span className="line-clamp-3">{activity}</span>
                      </div>
                    ))}
                  </div>
                </div>
