...
async function main(): Promise<void> {
  ...
  try {
    ...
    console.error("BEFORE-RM-HANDLES"); process._getActiveHandles().forEach((x,i)=>console.error("  ["+i+"] "+x.constructor?.name+" _handle="+(x._handle?.constructor?.name??"null")+" _isStdio="+(x._isStdio??"N/A"))); await rm(projectDir, { recursive: true, force: true }); console.error("RM-OK");
  }
}
